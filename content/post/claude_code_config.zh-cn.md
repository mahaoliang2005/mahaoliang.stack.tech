...
| **felo** | 3 | 实时网络搜索，获取最新信息和资讯 |
| **frontend-design** | 1 | 创建高质量前端界面 |
| **claude-hud:setup** | 1 | 配置 HUD 状态栏 |
| **cover-image-prompts** | 1 | 生成文章配图和封面图的 AI 绘图提示词 |

**drawio** 是我使用最多的 Skill，在讨论系统架构时特别有用——直接生成可编辑的图表文件，比文字描述直观得多。

**felo** 的使用场景很广泛——需要获取实时信息时（如最新技术动态、新闻事件、趋势数据），Claude 的知识可能已过时，这时通过 felo 进行实时搜索能补充最新上下文。
